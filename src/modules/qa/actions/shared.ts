import "server-only";

import { revalidatePath } from "next/cache";

import { getModule } from "@/config/modules";
import { getDictionary } from "@/lib/i18n/server";
import { interpolate } from "@/lib/i18n/interpolate";

import type { ActionResult } from "../types";

export function revalidateQa(projectId?: string) {
  revalidatePath(getModule("qa").href, "layout");
  revalidatePath(getModule("projects").href, "layout");
  if (projectId) revalidatePath(`${getModule("projects").href}/${projectId}`);
}

export async function fail(message: string | undefined): Promise<ActionResult> {
  const dict = await getDictionary();
  return { error: interpolate(dict.qa.errors.saveFailed, { message: message ?? "unknown" }) };
}

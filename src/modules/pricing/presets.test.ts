...
const person = (over: Partial<PersonPreset> & Pick<PersonPreset, "id" | "name">): PersonPreset => ({
  role: null, pricingModel: "hourly", hourlyCost: null, fixedPrice: null, marginPercent: null, currency: null, ...over,
});
...
const labels = { roleDefault: "role default", perHour: "/h", noRate: "no rate", fixedPerProject: "fixed", ofPrice: "of price" };

describe("resolvePreset", () => {
  it("fills a person's own rate, case-insensitively", () => {
    expect(resolvePreset("  jana nováková ", people, roles, "CZK")).toEqual({ kind: "person", name: "Jana Nováková", role: "Developer", payModel: "hourly", hourlyCost: 950, fixedPrice: null, percent: null });
  });
  ...
  it("resolves role defaults and unknown names", () => {
    expect(resolvePreset("developer", people, roles, "CZK")).toEqual({ kind: "role", name: "Developer", role: "Developer", payModel: "hourly", hourlyCost: 1100, fixedPrice: null, percent: null });
...
describe("presetOptions", () => {
  it("lists people (grouped by role) before role defaults, with pay model and rate", () => {
    ...
    expect(opts.map((o) => o.value)).toEqual(["Ota Fix", "Petr Smolin", "Jana Nováková", "Mia Berg", "Tomáš Kraus", "Sara Sales", "Developer", "3D Designer"]);
    expect(opts[0].label).toBe("3D Designer · fixed · 12000 CZK");
    expect(opts[1].label).toBe("3D Designer · 500 CZK/h");
    expect(opts[5].label).toBe("Sales · 10 % of price");
    expect(opts[6].label).toBe("role default · 1100 CZK/h");
  });
});

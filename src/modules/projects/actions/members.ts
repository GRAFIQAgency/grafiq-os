...
export async function addMember(projectId: string, raw: unknown): Promise<ActionResult> {
  ...
  const { error } = await supabase.from("project_members").insert({
    ...
    rate_source: m.rateSource, notes: m.notes,
  });
  if (error) return error.code === "23505" ? { error: dict.projects.errors.duplicateMember } : fail(error.message);
  await logActivity(supabase, [{ entityType: "project", entityId: projectId, action: "member_added", details: { name: displayName, role: m.projectRole, rate: m.costRate, source: m.rateSource } }], actor);
  revalidateProjects();
  return {};
}
